    ...
    contactForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          showNotification(
            "Mensagem enviada com sucesso! Recebemos sua solicitação.",
            "success"
          );
          contactForm.reset();
        } else {
          ...
          try {
            ...
            // Ignora se não for JSON válido (ex: erro 404 do Netlify)
          }

          const errorMessage =
            result?.error ||
            `Erro de Servidor (${response.status}). Verifique o ID do Formspree e a confirmação do e-mail.`;
        ...
        showNotification(
          "Ocorreu um erro de rede. Verifique sua conexão.",
          "error"
        );
      } finally {
